    ...
    <div class="px-8 pb-8 relative">
        <div class="flex flex-col md:flex-row md:items-end -mt-16 md:-mt-20 gap-8">
            <div class="relative group">
                <img src="{{ $user->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=2F6B3F&color=fff' }}" 
                ...
                <div class="absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl border-4 border-white flex items-center justify-center shadow-lg {{ $user->suspended_at ? 'bg-rose-500' : 'bg-emerald-500' }}">
                    <i class="fas {{ $user->suspended_at ? 'fa-user-slash' : 'fa-user-check' }} text-white text-xs"></i>
                </div>
            </div>
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-6 text-sm font-bold text-slate-500">
                            <span class="flex items-center gap-2"><i class="fas fa-envelope text-[#2F6B3F]"></i> {{ $user->email }}</span>
                            <span class="flex items-center gap-2"><i class="fas fa-calendar-alt text-[#2F6B3F]"></i> Joined {{ $user->created_at->format('M d, Y') }}</span>
                        </div>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        <a href="{{ route('admin.clients.edit', $user) }}" class="h-11 px-6 bg-slate-100 text-slate-700 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center shadow-sm">
                            <i class="fas fa-edit mr-2"></i> Edit Profile
                        </a>
                        <button onclick="showConfirmModal('{{ $user->suspended_at ? 'activate' : 'suspend' }}', '{{ route($user->suspended_at ? 'admin.client.activate' : 'admin.client.suspend', $user) }}', '{{ $user->name }}')" 
                                class="h-11 px-6 {{ $user->suspended_at ? 'bg-emerald-600' : 'bg-orange-600' }} text-white rounded-xl text-xs font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-lg flex items-center">
                            <i class="fas {{ $user->suspended_at ? 'fa-user-check' : 'fa-user-slash' }} mr-2"></i> {{ $user->suspended_at ? 'Restore Access' : 'Suspend Access' }}
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div class="flex justify-between items-center p-5 bg-emerald-50 rounded-2xl border border-emerald-100">
                    ...
                    <span class="text-2xl font-black text-emerald-800">{{ $stats['total_rides'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between items-center p-5 bg-[#2F6B3F] rounded-2xl shadow-lg">
                    ...
                    <span class="text-lg font-black text-white">TZS {{ number_format($user->rides_as_passenger->where('status', 'completed')->sum('fare'), 0) }}</span>
                </div>
            </div>
        </div>
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between">
                ...
                <a href="{{ route('admin.rides', ['search' => $user->name]) }}" class="h-9 px-4 bg-slate-100 hover:bg-[#2F6B3F] hover:text-white text-slate-600 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center">
                    Full Audit Log
                </a>
            </div>
            <div class="p-0">
                @if($user->rides_as_passenger->isEmpty())
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach($user->rides_as_passenger->take(10) as $ride)
        ...
        <div class="bg-rose-50 rounded-2xl shadow-lg border border-rose-200 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div class="flex flex-wrap gap-4">
                    <button onclick="showConfirmModal('delete', '{{ route('admin.client.delete', $user) }}', '{{ $user->name }}', 'DELETE')" 
